...
private:
    ...
    // 快照正在上传时不能追加日志，需要等待
    std::mutex snapshotInstallingMutex;

    // 由上层传入(KVServer)，保存对端的stub
    std::vector<std::shared_ptr<kvraft::KVRaftRPC_Stub>> peers;
    ...
    // 心跳定时器和选举定时器
    AfterTimer heartbeatsTimer;
    AfterTimer electionTimer;
    // 心跳间隔、选举超时时间
    double heartbeatsTime;
    ...
    std::vector<long long> nextIndex;
    std::vector<long long> matchIndex;

    // 快照最后一个一次操作对应日志的index和term
    long long lastSnapshotIndex;
    long long lastSnapshotTerm;
    ...
    std::string voterFor;
    // 保存在内存的日志
    std::vector<kvraft::LogEntry> logEntries;
    ...
    //启动raft服务 参数1：与其他对端通信用的stub 参数2：该服务器名字（唯一标识） 参数3：用来持久化 参数4：往上层提交达成共识的命令、快照
